...
{   
    ...
    public void Init(Color color, GameManager gameManager, Ctrl ctrl) {
        // 遍历子物体修改方块颜色
    ...
    private void Fall() {
        ...
        // 2. 位置上是否有 方块
        // 返回true 代表合法
        if (!ctrl.model.IsValidMapPosition(this.transform)){
            // 不合法 回退(上升一格)
            pos.y += 1f;
            transform.position = pos;
    ...
    public void FallComplete() {
        ...
        // 放置方块
        if (ctrl.model.PlaceShape(this.transform)) {
            // 如果有销毁播放销毁音效
            ctrl.audioManager.PlayLineclear();

            // 播放粒子特效
            if (Model.Instance.isParticles) {
                ctrl.view.CreateDestroyParticles();
            }
        }
    ...
    private void UpdateBlockPreview() {
        if (Model.Instance.isPreview) {
            ...
            while (true) {
                ...
                // 不行 回退+退出循环
                if (!ctrl.model.IsPreviewValidMapPosition(previewTier.transform)) {
                    downLength--;
    ...
    private void ColorStet1() {
        ctrl.model.horizontalMoveTime += Time.deltaTime;
        ...
        if (h != 0) {
            if(ctrl.model.horizontalMoveTime > ctrl.model.horizontalMoveTimeKey) {
                // 左右移动
                Vector3 pos = transform.position;
                pos.x += h;
                transform.position = pos;
                ...
                // 播放音效
                ctrl.audioManager.PlayControlMove();

                // 重置
                ctrl.model.horizontalMoveTime = 0f;
            }
        }
        ...
        if (Input.GetKey(KeyCode.DownArrow) || Input.GetKey(KeyCode.S)) {
            stepTime = ctrl.model.stepTimeS;
        } else {
            stepTime = stepTimeTemp;
        }

        // 直接下降
        if (Input.GetKeyDown(KeyCode.Space) && Model.Instance.isFallDirectly) {
            FallDirectly();
            ctrl.audioManager.PlayFallDirectly();
        }
    }
